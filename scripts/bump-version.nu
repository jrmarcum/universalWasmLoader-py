...
#
# Cross-platform sibling of bump-version.sh. Updates `[project] version` to the
# next semver, keeps pixi.toml's mirror in sync, commits the one-line change, and
# stops there — tagging/pushing is release.nu's job. Typical flow:
#
...
# Read the current [project] version from pyproject.toml (first version line).
...
# Replace only the first `version = "X.Y.Z"` line in a manifest file, in place.
def set-version [file: string, new: string] {
  let repl = ("${1}" + $new + "${2}")
  let newtext = (open --raw $file | str replace --regex '(?m)^(version\s*=\s*")[0-9]+\.[0-9]+\.[0-9]+(")' $repl)
  $newtext | save --raw --force $file
}
...
def main [
  ...
  if not ($cur =~ '^[0-9]+\.[0-9]+\.[0-9]+$') {
    print -e $"error: current version in pyproject.toml is not plain semver: '($cur)'"
    exit 1
  }
  ...
  # Apply: replace only the first version line, preserving formatting.
  print $"+ set version = \"($new)\" in pyproject.toml \(and pixi.toml mirror)"
  if not $dry_run {
    set-version pyproject.toml $new
    if ("pixi.toml" | path exists) { set-version pixi.toml $new }
  }

  if not $no_commit {
    print "+ git add pyproject.toml pixi.toml"
    if not $dry_run {
      ^git add pyproject.toml
      if ("pixi.toml" | path exists) { ^git add pixi.toml }
    }
